(()=>{
  ...
  const isReta=a=>{
    ...
    return /^Reta Final GCM(?: Paracatu)?(?:\s*[↗→])?$/i.test(t)||/reta-final-gcm-paracatu/i.test(h);
  };

  // Limpa links indesejados e qualquer duplicata antiga em TODOS os blocos do header.
  [nav,social,mobile].filter(Boolean).forEach(root=>{
    [...root.querySelectorAll('a')].forEach(a=>{
      if(isWhatsapp(a)||/plano genival/i.test(norm(a.textContent))) a.remove();
    });
  });
  inner.querySelector('.mcp-header-whatsapp')?.remove();

  // Medicina Legal: um único link, apontando para o cronograma Genival.
  const meds=[];
  [nav,social,mobile].filter(Boolean).forEach(root=>{
    [...root.querySelectorAll('a')].forEach(a=>{
      if(/^Medicina Legal$/i.test(norm(a.textContent))||/plano-leitura-medicina-legal-genival/i.test(a.getAttribute('href')||'')) meds.push(a);
    });
  });
  ...
  // Instagram e PP-RN sempre no final.
  ...
  const pprn=[...nav.querySelectorAll(':scope > a')].find(a=>/^PP-RN 2026$/i.test(norm(a.textContent)));
  if(pprn){pprn.dataset.mcpLowPriority='1';nav.appendChild(pprn);}
  [...nav.querySelectorAll(':scope > a')].forEach(a=>{if(/^Editais$/i.test(norm(a.textContent)))a.dataset.mcpLowPriority='1';});

  // Garantia extra: depois de reorganizar, qualquer variante remanescente é eliminada.
  const finalRetas=[...nav.querySelectorAll(':scope > a')].filter(isReta);
  finalRetas.forEach(a=>{if(a!==reta)a.remove();});

  const style=document.createElement('style');
  style.id='mcp-header-fix-final';
  style.textContent=`
  ...
  // Observa alterações tardias no header e mata duplicatas que algum script antigo tentar recriar.
  const observer=new MutationObserver(()=>{
    const all=[...nav.querySelectorAll(':scope > a')].filter(isReta);
